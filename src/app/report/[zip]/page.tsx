...
function Analyzing({ zip }: { zip: string }) { return <main className="mx-auto flex min-h-[70vh] max-w-xl flex-col justify-center px-6 text-center"><div className="mx-auto grid size-16 place-items-center rounded-full bg-teal-100 text-3xl text-teal-700 animate-pulse">≈</div><p className="mt-7 text-xl font-semibold text-slate-900">Analyzing 30 years of EPA data…</p><p className="mt-2 leading-7 text-slate-500">We&apos;re connecting public records to the water system serving ZIP {zip}.</p></main>; }
function NotFound({ zip }: { zip: string }) { const suggestions = ["07030", "12866", "10001"]; return <main className="mx-auto flex min-h-[70vh] max-w-xl flex-col justify-center px-6 text-center"><div className="mx-auto grid size-16 place-items-center rounded-full bg-sky-100 text-3xl">⌕</div><h1 className="mt-7 text-3xl font-semibold tracking-tight text-slate-900">We couldn&apos;t find a report for {zip}</h1><p className="mt-3 leading-7 text-slate-600">EPA coverage can be incomplete for some ZIP codes. Try checking the ZIP again or explore a sample report.</p><div className="mt-7 flex justify-center gap-2">{suggestions.map((sample) => <Link key={sample} href={`/report/${sample}`} className="rounded-full border border-teal-200 bg-white px-3 py-2 text-sm font-medium text-teal-800">{sample}</Link>)}</div></main>; }
