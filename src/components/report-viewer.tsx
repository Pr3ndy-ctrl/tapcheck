...
export function ReportViewer({ zip, systems }: { zip: string; systems: SystemReport[] }) {
  ...
  return <main className="mx-auto max-w-5xl px-5 py-10 sm:px-8 sm:py-14">
    <div className="mb-9 flex items-center justify-between"><Link href="/" className="flex items-center gap-2 text-xl font-semibold tracking-tight text-teal-900"><span className="grid size-8 place-items-center rounded-xl bg-teal-600 text-sm text-white">≈</span>TapCheck</Link><Link href="/" className="text-sm font-medium text-teal-700 hover:text-teal-900">Check another ZIP</Link></div>
    <p className="mb-3 text-sm font-semibold uppercase tracking-[0.16em] text-teal-700">Water report · ZIP {zip}</p>
    {systems.length > 1 && <div className="mb-6 flex gap-2 overflow-x-auto pb-1" role="tablist">{systems.map((entry, index) => <button key={entry.system.pwsid} role="tab" aria-selected={active === index} onClick={() => setActive(index)} className={`shrink-0 rounded-full border px-4 py-2 text-sm font-medium transition ${active === index ? "border-teal-700 bg-teal-700 text-white" : "border-slate-200 bg-white text-slate-600 hover:border-teal-300"}`}>{entry.system.name ?? entry.system.pwsid}</button>)}</div>}
    <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-[0_24px_70px_-45px_rgba(15,118,110,0.45)]">
      <div className="bg-gradient-to-br from-teal-50 via-white to-cyan-50 p-7 sm:p-10">
...
function NonHuman({ report }: { report: AiWaterReport }) { const labels = { fish: ["🐟", "Fish"], plants: ["🪴", "Plants"], pets: ["🐾", "Pets"] } as const; return <section><p className="text-sm font-semibold uppercase tracking-[0.15em] text-teal-700">Not just for humans</p><h2 className="mt-1 text-2xl font-semibold tracking-tight text-slate-900">How it may affect the rest of the household</h2><div className="mt-5 grid gap-4 md:grid-cols-3">{(Object.keys(labels) as Array<keyof typeof labels>).map((key) => { const [icon, label] = labels[key]; const advice = report.nonHuman[key]; return <article key={key} className="rounded-2xl border border-slate-200 p-5"><div className="flex items-center justify-between"><h3 className="font-semibold text-slate-900">{icon} {label}</h3><span className={`rounded-full px-2.5 py-1 text-xs font-semibold capitalize ${safetyStyle[advice.safety]}`}>{advice.safety}</span></div><p className="mt-3 text-sm leading-6 text-slate-600">{advice.explanation}</p></article>; })}</div></section>; }
